"""
AbbasBot Pairs -- forex pairs signal scanner (USD/CAD, GBP/USD, AUD/JPY)
with trade tracking, auto win-rate stats, and quiet messaging -> Telegram
alerts. Same strategy/logic as AbbasBot, just a separate bot/chat/state
file so it runs fully independently and only covers pairs with a
confirmed backtested edge (no Gold here -- see AbbasBot for that).
...
STATE_FILE = "trades_state_pairs.json"
...
PAIR_CURRENCIES = {
    ...
    "NZD/USD": {"NZD", "USD"}, "XAU/USD": {"USD"}, "AUD/JPY": {"AUD", "JPY"},
}
...
RANKING_FILE = "pair_ranking_pairs.json"  # separate from AbbasBot's ranking file -- different pair universe
